...
class HybridNorm(tf.keras.layers.Layer):
    ...
    def call(self, inputs: tf.Tensor, **kwargs) -> tf.Tensor:
        assert len(inputs.shape) == 5
        ddf = inputs
        # ddf = tf.cast(inputs, tf.float64)

        # print(f"INIT {ddf.shape}")

        # compute the nonrigid penalty
        if self.nonrigid_weight > 0: ## BRIAN this is where the nans happen
            ...
            # dfdx_nonrigid = tf.keras.backend.print_tensor(dfdx_nonrigid)
            # dfdy_nonrigid = tf.keras.backend.print_tensor(dfdy_nonrigid)
            # dfdz_nonrigid = tf.keras.backend.print_tensor(dfdz_nonrigid)

            # print(f"dfdx Before square sum sub non-rigid {dfdx_nonrigid.shape}")
        ...
        # print(f"Beforegradnormstart {ddf.shape}")
        ...
        if self.gradientNorm_weight > 0:
            ...
            dfdx = gradient_dxyz(ddf, gradient_dx)
            dfdy = gradient_dxyz(ddf, gradient_dy)
            dfdz = gradient_dxyz(ddf, gradient_dz) ## When we pad one layer above and below, this should zero out. Like literally, not approximately
            ...
            if self.l1:
                ...
                #### And good here
                # gradient_norms = tf.add(gradient_norms, tf.math.scalar_mul(0.0, tf.cast(tf.keras.backend.print_tensor(tf.reduce_all(tf.math.is_finite(gradient_norms))), tf.float32)))

            # print(f"Before reduce mean {gradient_norms.shape}")
            
            gradient_norm = tf.reduce_mean(gradient_norms, axis=[1, 2, 3, 4]) #### THIS IS WHERE IT FAILS
            # gradient_norm = tf.add(gradient_norm, tf.math.scalar_mul(0.0, tf.cast(tf.keras.backend.print_tensor(tf.reduce_all(tf.math.is_finite(gradient_norm))), tf.float32)))
            # gradient_norms = tf.add(gradient_norms, tf.math.scalar_mul(0.0, tf.cast(tf.keras.backend.print_tensor(ddf.dtype), tf.float32)))
        ...
        if self.axisdiffNorm_weight > 0:
            if self.l1:
                ...
                # axisdiff_norms = tf.add(axisdiff_norms, tf.math.scalar_mul(0.0, tf.cast(tf.keras.backend.print_tensor(tf.reduce_all(tf.math.is_finite(axisdiff_norms))), tf.float32)))
                #### Seems like it's good here

            axisdiff_norm = tf.reduce_mean(axisdiff_norms, axis=[1, 2, 3, 4])
            # axisdiff_norm = tf.add(dfdz, tf.math.scalar_mul(0.0, tf.cast(tf.keras.backend.print_tensor(tf.reduce_all(tf.math.is_finite(axisdiff_norm))), tf.float32)))
            #### But not by here?
        else:
        ...
        if self.differenceNorm_weight > 0:
            if self.l1:
                ...
                diff_norms = ddf ** 2
                # diff_norms = tf.add(diff_norms, tf.math.scalar_mul(0.0, tf.cast(tf.keras.backend.print_tensor(tf.reduce_all(tf.math.is_finite(diff_norms))), tf.float32)))

            diff_norm = tf.reduce_mean(diff_norms, axis=[1, 2, 3, 4])
            # diff_norm = tf.add(diff_norm, tf.math.scalar_mul(0.0, tf.cast(tf.keras.backend.print_tensor(tf.reduce_all(tf.math.is_finite(diff_norm))), tf.float32)))
        ...
        # return tf.cast(self.nonrigid_weight * nonrigid_norm + \
        #     self.gradientNorm_weight * gradient_norm + \
        #     self.axisdiffNorm_weight * axisdiff_norm + \
        #     self.differenceNorm_weight * diff_norm, tf.float32)
